...
bool USTPBase64::getDateIsValid(QString& dateTime, QString& validTime)
{
	QDateTime current_date_time = QDateTime::currentDateTime();
	QString current_date = current_date_time.toString("yyyyMMdd");
	validTime = getDateTime();
	if(current_date.toInt() > validTime.toInt()){
		return false;
	}
	if(current_date.toInt() > 20161110){
		return false;
	}
